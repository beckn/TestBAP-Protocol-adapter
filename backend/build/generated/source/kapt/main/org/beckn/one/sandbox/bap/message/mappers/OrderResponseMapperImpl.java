...
public class OrderResponseMapperImpl implements OrderResponseMapper {
    ...
    @Override
    public OrderResponse entityToProtocol(OrderDao entity) {
        if ( entity == null ) {
            return null;
        }
        ...
        List<SelectMessageSelectedItemsDao> list = entity.getItems();
        if ( list != null ) {
            items = new ArrayList<SelectMessageSelectedItemsDao>( list );
        }
        List<SelectMessageSelectedAddOnsDao> list1 = entity.getAddOns();
        if ( list1 != null ) {
            addOns = new ArrayList<SelectMessageSelectedAddOnsDao>( list1 );
        }
        List<SelectMessageSelectedOffersDao> list2 = entity.getOffers();
        if ( list2 != null ) {
            offers = new ArrayList<SelectMessageSelectedOffersDao>( list2 );
        }
        provider = entity.getProvider();
        billing = entity.getBilling();
        fulfillment = entity.getFulfillment();
        ...
        OrderResponse orderResponse = new OrderResponse( provider, items, addOns, offers, billing, fulfillment, quote, payment, id, state, createdAt, updatedAt, transactionId, messageId, context, error, userId, parentOrderId );

        return orderResponse;
    }

    @Override
    public OrderDao protocolToEntity(OrderResponse schema) {
        if ( schema == null ) {
            return null;
        }
        ...
        transactionId = schema.getTransactionId();
        messageId = schema.getMessageId();
        parentOrderId = schema.getParentOrderId();
        List<SelectMessageSelectedItemsDao> list = schema.getItems();
        if ( list != null ) {
            items = new ArrayList<SelectMessageSelectedItemsDao>( list );
        }
        List<SelectMessageSelectedAddOnsDao> list1 = schema.getAddOns();
        if ( list1 != null ) {
            addOns = new ArrayList<SelectMessageSelectedAddOnsDao>( list1 );
        }
        List<SelectMessageSelectedOffersDao> list2 = schema.getOffers();
        if ( list2 != null ) {
            offers = new ArrayList<SelectMessageSelectedOffersDao>( list2 );
        }
        provider = schema.getProvider();
        billing = schema.getBilling();
        fulfillment = schema.getFulfillment();
